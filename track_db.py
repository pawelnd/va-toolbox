...
# Function to populate the relation_map with relation id, table names, and column names
    ...
    try:
        with psycopg2.connect(**conn_params) as metadata_conn:
            with metadata_conn.cursor() as cursor:
                ...
                # print("Relation map updated:", relation_map)
    except psycopg2.Error as e:
        print("Error fetching relation map:", e)

# Define connection details
conn_params = {
    'dbname': 'butterfly',    # Your database name
    'user': 'btfluser',       # Your PostgreSQL username
    'password': 'btfluser',   # Your PostgreSQL password
    'host': 'localhost',      # Your PostgreSQL host (localhost)
    'port': '5433'            # Your PostgreSQL port (5433)
}
def clear_console():
...
def decode_pgoutput_message(payload):
    ...
    if payload[0:1].decode('ascii') == 'B':  # Transaction BEGIN (optional)
        ...
        # Clear console on transaction BEGIN
        # clear_console()
        return
    if message_type == 'I':  # INSERT message
        ...
        # 1 byte for the 'N' (new tuple follows)
        if rest[0:1].decode('ascii') == 'N':
            ...
            for col_idx in range(column_count):
                ...
                if isnull == b't':  # 't' means the column is not null
                    offset += 1
        ...
        # Before updating, there is a 1-byte code to indicate whether this is the "key" or "old" tuple.
        ...
        if rest[0:1].decode('ascii') == 'N':
            ...
            rest = rest[1:]
            column_count = struct.unpack('!H', rest[0:2])[0]  # Number of columns (2-byte integer)
            offset = 2
        ...
        # 'K' indicates a key tuple, necessary to identify the row being deleted
        if rest[0:1].decode('ascii') == 'K':
...
def stream_changes():
    with conn.cursor() as cur:
        ...
        cur.start_replication(
            slot_name='my_slot',
            options={
                'proto_version': '1',               # Protocol version should be 1
                'publication_names': 'my_publication'  # Use your publication name
            }
        )
        ...
        try:
            cur.consume_stream(consume_change)  # Start consuming the stream
